...
if __name__ == "__main__":
    ...
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='Bunkr文件下载器')
    parser.add_argument('url', help='要下载的bunkr URL')
    parser.add_argument('--download-dir', '-d', default='downloads', help='下载目录 (默认: downloads)')
    parser.add_argument('--ignore', nargs='+', help='忽略包含这些模式的文件')
    parser.add_argument('--include', nargs='+', help='只下载包含这些模式的文件')
    parser.add_argument('--async', action='store_true', help='使用异步下载')
    ...
    # 输出结果
    print(f"\n下载完成！")
    print(f"成功下载: {result['files_downloaded']} 个文件")
    print(f"下载失败: {result['files_failed']} 个文件")
    print(f"跳过文件: {len(result['skipped_files'])} 个文件")
    
    if result['downloaded_files']:
        print(f"已下载的文件: {result['downloaded_files']}")
